...
export default function Security({ cryptoKey }: Props) {
  ...
  const strong = credentials.filter(
    (c) => getStrength(c.password) === "strong"
  );
    ...
    <div className="min-h-screen bg-[#0a0a0f]">
      <div className="max-w-lg mx-auto px-4 py-6">
        <div className="flex items-center gap-3 mb-6">
          ...
          <h1 className="text-white font-semibold">Безопасность</h1>
        </div>
        ...
        >
          ...
          <div className="bg-[#13131a] border border-white/5 rounded-2xl p-5">
            ...
            <div className="flex gap-2 mb-3">
              {[
                ...
                { count: weak.length, label: "Слабых", color: "bg-red-400" },
              ].map(
                (item) =>
                  credentials.length > 0 && (
                    <div
                      ...
                      style={{
                        width: `${(item.count / credentials.length) * 100}%`,
            ...
            <div className="grid grid-cols-3 gap-2">
              {[
                ...
                { count: weak.length, label: "Слабых", color: "text-red-400" },
              ].map((item) => (
          ...
          {weak.length > 0 && (
            <div className="bg-[#13131a] border border-red-500/10 rounded-2xl p-5">
              <h2 className="text-red-400 text-sm font-medium mb-3">
                ⚠ Слабые пароли
              </h2>
              <div className="flex flex-col gap-2">
                {weak.map((c) => (
                  ...
                  >
                    ...
                    <span className="text-zinc-600 text-xs">Исправить →</span>
                  </button>
                ))}
              </div>
            </div>
          ...
          {reusedGroups.length > 0 && (
            <div className="bg-[#13131a] border border-orange-500/10 rounded-2xl p-5">
              <h2 className="text-orange-400 text-sm font-medium mb-3">
          ...
          {weak.length === 0 &&
            ...
            credentials.length > 0 && (
              <div className="bg-[#13131a] border border-green-500/10 rounded-2xl p-5 text-center">
                ...
                <p className="text-white text-sm font-medium">Всё в порядке</p>
                <p className="text-zinc-500 text-xs mt-1">
                  Все пароли уникальны и достаточно сильны
                </p>
              </div>
            )}
        </motion.div>
